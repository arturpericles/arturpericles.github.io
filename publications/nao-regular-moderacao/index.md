---
title: 'Como não regular a moderação de conteúdo: lições de uma tentativa fracassada'
date: 2022-01-01
title_en: 'How not to regulate content moderation: Lessons from a failed attempt'
container-title_en: 'Freedom of Expression: A Contested Concept'
author:
  - given: Artur Pericles
    family: Lima Monteiro
    affiliations:
      - name: 'Yale Law School & Jackson School of Global Affairs'
citation:
  type: chapter
  language: pt-BR
  author:
    - given: Artur Pericles
      family: Monteiro
      dropping-particle: L.
  citation-key: APLM.2022hng
  publisher: Jota
  title: 'Como não regular a moderação de conteúdo: lições de uma tentativa fracassada'
  container-title: 'Liberdade de expressão: um conceito em disputa'
fulltext: /media/preprints/monteiro-como_nao_regular_moderacao.pdf
---
